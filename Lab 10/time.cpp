...
bool timeOverlap(TimeSlot ts1, TimeSlot ts2){
    TimeSlotInterval tsi1 = {ts1.startTime, addMinutes(ts1.startTime, ts1.movie.duration)};
    TimeSlotInterval tsi2 = {ts2.startTime, addMinutes(ts2.startTime, ts2.movie.duration)};
    if(minutesSinceMidnight(ts1.startTime) > minutesSinceMidnight(tsi2.startTime)){
        TimeSlotInterval tsi2 = {ts1.startTime, addMinutes(ts1.startTime, ts1.movie.duration)};
        TimeSlotInterval tsi1 = {ts2.startTime, addMinutes(ts2.startTime, ts2.movie.duration)};
    }
    if(minutesSinceMidnight(tsi2.startTime) <= minutesSinceMidnight(tsi1.endTime)) {return true;}
    return false;
}
